...
public class CommentViewService {
    ...
    public List<Comment> getCommentViewList(Long crewId) {
        List<Comment> list = commentRepository.findByCrewId(crewId);
        return list;
    }
    @Transactional(readOnly = true)
    public List<Comment> getChildCommentViewList(Long crewId, Long parentCommentId) {
        List<Comment> list = commentRepository.findByCrewIdAndParentId(crewId, parentCommentId);
        return list;
    }

    // 대댓글 작성 - view
    public CommentResponse addChildComment(CommentRequest commentRequest, Long parentCommentId, String userName) {
